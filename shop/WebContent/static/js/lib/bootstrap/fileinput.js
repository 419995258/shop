...
(function($) {
	...
	$.fn.fileinput.locales.en = {
		/*
		 * fileSingle: 'file', filePlural: 'files', browseLabel: 'Browse
		 * &hellip;', removeLabel: 'Remove', removeTitle: 'Clear selected
		 * files', cancelLabel: 'Cancel', cancelTitle: 'Abort ongoing upload',
		 * uploadLabel: 'Upload', uploadTitle: 'Upload selected files',
		 * msgSizeTooLarge: 'File "{name}" (<b>{size} KB</b>) exceeds maximum
		 * allowed upload size of <b>{maxSize} KB</b>. Please retry your
		 * upload!', msgFilesTooLess: 'You must select at least <b>{n}</b>
		 * {files} to upload. Please retry your upload!', msgFilesTooMany:
		 * 'Number of files selected for upload <b>({n})</b> exceeds maximum
		 * allowed limit of <b>{m}</b>. Please retry your upload!',
		 * msgFileNotFound: 'File "{name}" not found!', msgFileSecured:
		 * 'Security restrictions prevent reading the file "{name}".',
		 * msgFileNotReadable: 'File "{name}" is not readable.',
		 * msgFilePreviewAborted: 'File preview aborted for "{name}".',
		 * msgFilePreviewError: 'An error occurred while reading the file
		 ...
		 * supported.', msgValidationError: 'File Upload Error', msgLoading:
		 * 'Loading file {index} of {files} &hellip;', msgProgress: 'Loading
		 * file {index} of {files} - {name} - {percent}% completed.',
		 * msgSelected: '{n} {files} selected', msgFoldersNotAllowed: 'Drag &
		 * drop files only! {n} folder(s) dropped were skipped.', dropZoneTitle:
		...
		slugCallback : function(text) {
			return text ? text.split(/(\\|\/)/g).pop().replace(
					/[^\w\u4e00-\u9fa5\-.\\\/ ]+/g, '') : '';
		}
	};
