var allOrderAppmodule = angular.module('allOrderApp', []);

allOrderAppmodule.controller('allOrderController', function($scope, $http, $rootScope, $stateParams, $state) {		
	...
	$scope.init = function() {
		if ($state.current.name == 'allOrder') {
	...
	function queryWaitOrder() {
		...
		$http({
			method : "POST",
			url : "../ordermanageC/queryWaitOrder",
			data : resultVo
	...
	/**********************************************确认取消*******************************************/
	$scope.deleteOrder = function() { // 点击按钮
		$scope.jkdIndex = delIdTemp; // 设置选中行索引

		$http({
			method : "PUT",
			url : "../ordermanageC/deleteOrder",
			data : $scope.orderMini[$scope.jkdIndex]
		}).success(function(data, status) {
			$scope.orderMini.splice(delIdTemp, 1);
			delIdTemp = '';
	...
	/**********************************************订单完成确认*******************************************/
	$scope.openFinishPage = function(index) {
		delIdTemp = index;
		$('#finishModal').modal('show');
	};
	/**********************************************确认完成订单*******************************************/
	$scope.sureOrder = function() { // 点击按钮
		$scope.jkdIndex = delIdTemp; // 设置选中行索引

		$http({
			method : "PUT",
			url : "../ordermanageC/sureOrder",
			data : $scope.orderMini[$scope.jkdIndex]
		}).success(function(data, status) {
			$scope.orderMini.splice(delIdTemp, 1);
			delIdTemp = '';
